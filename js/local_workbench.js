(function () {
  ...
  function initVideoCut(host) {
    // 使用原生浏览器 API 实现，无需 FFmpeg.wasm，避免跨域问题
    if (typeof window.initVideoCutNative === 'function') {
      return window.initVideoCutNative(host);
    }
    
    // 备用：显示提示信息
    host.innerHTML = `
      <div class="tool-actions">
        <p class="hint" style="padding: 20px; text-align: center;">
          视频剪辑工具加载中...<br>
          <small>如果长时间未加载，请刷新页面或检查 js/video_cut.js 文件</small>
        </p>
      </div>
    `;
  }
