...
export WIKI_AI_MODEL="${WIKI_AI_MODEL:-deepseek-flash}"
export WIKI_AI_FILTER="${WIKI_AI_FILTER:-1}"
export WIKI_MIN_RELEVANCE_SCORE="${WIKI_MIN_RELEVANCE_SCORE:-5}"
export WIKI_MAX_PER_SOURCE="${WIKI_MAX_PER_SOURCE:-8}"
export WIKI_AI_MAX_ENTRIES="${WIKI_AI_MAX_ENTRIES:-40}"
export WIKI_PRESCREEN_MIN="${WIKI_PRESCREEN_MIN:-2}"
...
if command -v opencode >/dev/null 2>&1; then
  ...
  if command -v script >/dev/null 2>&1; then
    ...
    timeout 180s opencode run "$prompt" --auto -m "deepseek/deepseek-flash" --dir "$SITE" > "$OPEN_CODE_LOG" 2>&1 || opencode_status=$?
  fi
